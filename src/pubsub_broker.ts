 ...
 * トピック
 */
type PubSubTopic = symbol | string;
...
/**
  ...
  */
class PubSubBroker<T> {
  /**
  * 購読の登録簿
  */
  readonly #subscriptions: Map<PubSubTopic, Map<(message: T) => Promise<void>, SubscriptionOptions>>;
  ...
  */
  subscribe(topic: PubSubTopic, callback: (message: T) => Promise<void>, options: SubscriptionOptions = {}): void {
  ...
  */
  unsubscribe(topic: PubSubTopic, callback: (message: T) => Promise<void>): void {
  ...
  * @param message - メッセージ
  */
  async publish(topic: PubSubTopic, message: T): Promise<void> {
    if (this.#subscriptions.has(topic)) {
      const topicSubscriptions = this.#subscriptions.get(topic) as Map<(message: T) => Promise<void>, SubscriptionOptions>;

...
import { IsomorphicEncoding } from "../src/isomorphic_encoding.ts";
...
Deno.test("IsomorphicEncoding.decode", () => {
  ...
  assertStrictEquals(IsomorphicEncoding.decode(Uint8Array.of(0x41,0x42,0x43,0x44).buffer), "ABCD");

  // decode(Uint8Array)
  assertStrictEquals(IsomorphicEncoding.decode(Uint8Array.of()), "");
  assertStrictEquals(IsomorphicEncoding.decode(Uint8Array.of(0x41,0x42,0x43,0x44)), "ABCD");
  assertStrictEquals(IsomorphicEncoding.decode(Uint8Array.of(0x0,0xFF)), "\u0000\u00FF");
...
Deno.test("IsomorphicEncoding.encode", () => {
  ...
  // encode(string)
  assertStrictEquals(JSON.stringify([...IsomorphicEncoding.encode("")]), "[]");
  assertStrictEquals(JSON.stringify([...IsomorphicEncoding.encode("ABCD")]), "[65,66,67,68]");
  assertStrictEquals(JSON.stringify([...IsomorphicEncoding.encode("\u0000\u00FF")]), "[0,255]");

...
Deno.test("ByteSequence.prototype.loadFromUint16AsyncIterable()", async () => {
  async function* bs() {
    yield 0xFFF0;
    yield 0x0033;
  }
  ...
  await b.loadFromUint16AsyncIterable(bs());
  ...
  await b2.loadFromUint16AsyncIterable(bs(), { byteOrder: "big-endian" });
  ...
  await b3.loadFromUint16AsyncIterable(bs(), { byteOrder: "little-endian" });
...
Deno.test("ByteSequence.prototype.loadFromUint16AsyncIterable() - error", async () => {
  const b3 = ByteSequence.create(4);
  await assertRejects(
    async () => {
      await b3.loadFromUint16AsyncIterable(
...
Deno.test("ByteSequence.prototype.loadFromUint16AsyncIterable() - insertAt", async () => {
  async function* bs() {
    yield 0xFFF0;
    yield 0x0033;
  }
  ...
  await b.loadFromUint16AsyncIterable(bs());
  await b.loadFromUint16AsyncIterable(bs(), { insertAt: 1 });
  const bytes = new Uint8Array(b.toArrayBufferWithDetach());

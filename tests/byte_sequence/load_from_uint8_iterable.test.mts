...
Deno.test("ByteSequence.prototype.loadFromUint8Iterable()", () => {
  const b = ByteSequence.create(4);
  b.loadFromUint8Iterable([255, 254, 253]);
  ...
  b2.loadFromUint8Iterable(Uint8Array.of(255, 254, 253));
...
Deno.test("ByteSequence.prototype.loadFromUint8Iterable() - error", () => {
  const b = ByteSequence.create(4);
  assertThrows(
    () => {
      b.loadFromUint8Iterable(["255", "254", "253"] as unknown as number[]);
    },
    TypeError,
  ...
  assertThrows(
    () => {
      b2.loadFromUint8Iterable("255" as unknown as number[]);
    },
    TypeError,
    "Input must be a safe-integer of type `number`", //XXX 主語を変えたい
  );
  ...
  assertThrows(
    () => {
      b3.loadFromUint8Iterable(255 as unknown as number[]);
    },
    TypeError,
...
Deno.test("ByteSequence.prototype.loadFromUint8Iterable() - insertAt", () => {
  const b = ByteSequence.create(8);
  b.loadFromUint8Iterable([255, 254, 253]);
  b.loadFromUint8Iterable([255, 254, 253], { insertAt: 1 });
  const bytes = new Uint8Array(b.toArrayBufferWithDetach());
  ...
  assertThrows(
    () => {
      b2.loadFromUint8Iterable(Uint8Array.of(255, 254, 253), { insertAt: 10 });
    },
    RangeError,

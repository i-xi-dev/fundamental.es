...
Deno.test("ByteSequence.Digest.Md5.compute()", async () => {
  ...
  const s1 = await ByteSequence.Digest.Md5.compute(new Uint8Array(1));
  assertStrictEquals(s1.toHex(), "93b885adfe0da089cdf634904fd59f71");

  const s2 = await ByteSequence.Digest.Md5.compute(new Uint8Array(2));
  assertStrictEquals(s2.toHex(), "c4103f122d27677c9db144cae1394a66");

  const s3 = await ByteSequence.Digest.Md5.compute(new Uint8Array(3));
  assertStrictEquals(s3.toHex(), "693e9af84d3dfcc71e640e005bdc5e2e");

  const s4 = await ByteSequence.Digest.Md5.compute(new Uint8Array(4));
  assertStrictEquals(s4.toHex(), "f1d3ff8443297732862df21dc4e57262");

  const s5 = await ByteSequence.Digest.Md5.compute(new Uint8Array(5));
  assertStrictEquals(s5.toHex(), "ca9c491ac66b2c62500882e93f3719a8");

  const s6 = await ByteSequence.Digest.Md5.compute(new Uint8Array(6));
  assertStrictEquals(s6.toHex(), "7319468847d7b1aee40dbf5dd963c999");

  const s55 = await ByteSequence.Digest.Md5.compute(new Uint8Array(55));
  assertStrictEquals(s55.toHex(), "c9ea3314b91c9fd4e38f9432064fd1f2");

  const s56 = await ByteSequence.Digest.Md5.compute(new Uint8Array(56));
  assertStrictEquals(s56.toHex(), "e3c4dd21a9171fd39d208efa09bf7883");

  const s119 = await ByteSequence.Digest.Md5.compute(new Uint8Array(119));
  assertStrictEquals(s119.toHex(), "8271cb2e6a546123b43096a2efce39d2");

  const s120 = await ByteSequence.Digest.Md5.compute(new Uint8Array(120));
  assertStrictEquals(s120.toHex(), "222f7d881ded1871724a1b9a1cb94247");

  assertStrictEquals(
    (await ByteSequence.Digest.Md5.compute(Uint8Array.of(1, 2, 3, 4))).toHex(),
    "08d6c05a21512a79a1dfeb9d2a8f262f",
  );
  assertStrictEquals(
    (await ByteSequence.Digest.Md5.compute(
  ...
  const md51 = new _md5();
  const r11 = (await ByteSequence.Digest.Md5.compute(src1)).toHex();
  ...
  const md52 = new _md5();
  const r12 = (await ByteSequence.Digest.Md5.compute(src2)).toHex();
  ...
  const md53 = new _md5();
  const r13 = (await ByteSequence.Digest.Md5.compute(src3)).toHex();
  ...
  const md54 = new _md5();
  const r14 = (await ByteSequence.Digest.Md5.compute(src4)).toHex();

...
type DecoderOptions = ByteSequence.Encoding.Base64.DecoderOptions;
const { DecoderStream } = ByteSequence.Encoding.Base64;
...
Deno.test("ByteSequence.Encoding.Base64.DecoderStream", async () => {
  await test1(
    ["AwIBAP/+/fw="],
...
Deno.test("ByteSequence.Encoding.Base64.DecoderStream - alphabet:base64url", async () => {
  await test1(
    ["AwIBAP_-_fw="],
    Uint8Array.of(0x03, 0x02, 0x01, 0x00, 0xFF, 0xFE, 0xFD, 0xFC, 0x00, 0x00),
    { alphabet: "base64url" },
...
Deno.test("ByteSequence.Encoding.Base64.DecoderStream - lastChunkHandling:strict", async () => {
  await test1(
    ["AwIBAP/+/fw="],
    Uint8Array.of(0x03, 0x02, 0x01, 0x00, 0xFF, 0xFE, 0xFD, 0xFC, 0x00, 0x00),
...
Deno.test("ByteSequence.Encoding.Base64.DecoderStream - error", async () => {
  await assertRejects(
    async () => {
      await test1(
        ["A", "w", "あ", "B", "A", "P", "/", "+", "/", "f", "w", "="],

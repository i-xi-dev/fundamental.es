...
const { DecoderStream } = ByteSequence.Encoding.Hex;
...
Deno.test("ByteSequence.Encoding.Hex.DecoderStream", async () => {
  await test1(
...
Deno.test("ByteSequence.Encoding.Hex.DecoderStream - error", async () => {
  await assertRejects(
    async () => {
      await test1(
        [

...
async function test1(
  ...
  expected: string,
  encoderOptions?: Base64.EncoderOptions,
): Promise<void> {
  ...
  const encoder = new Base64.EncoderStream(encoderOptions);
...
Deno.test("Base64.EncoderStream", async () => {
  await test1([
...
Deno.test("Base64.EncoderStream - alphabet:base64url", async () => {
  await test1(
    [
      Uint8Array.of(0x03, 0x02, 0x01, 0x00, 0xFF, 0xFE, 0xFD, 0xFC),
    ],
...
Deno.test("Base64.EncoderStream - omitPadding:true", async () => {
  await test1(
    [
      Uint8Array.of(0x03, 0x02, 0x01, 0x00, 0xFF, 0xFE, 0xFD, 0xFC),
    ],
